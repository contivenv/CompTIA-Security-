...
**Cenário:** Aplicação de um _patch_ crítico de segurança no servidor de banco de dados de um sistema hospitalar.
...
3. **Execução e Validação:** O serviço de banco de dados é interrompido, o _patch_ é instalado e o servidor é reiniciado.
    
4. **Plano de Reversão:** Se, após a reinicialização, o sistema de triagem não conseguir se reconectar ao banco de dados, a equipe aborta a atualização e restaura o _snapshot_ do servidor imediatamente, garantindo que a operação retorne ao estado funcional antes do fim da janela de manutenção.